...
Route<void> get formRoute => MaterialPageRoute(builder: (_) => const _FormPage());
  ...
  Widget build(BuildContext context) {
    return GestureDetector(
      ...
      child: PTScaffold(
        ...
        top: const TopBar(),
        body: _submitted ? const _CountdownTimer() : _Form(onSubmit: _showTimer),
      ),
    );
  }
...
class _FormState extends State<_Form> {
      ...
      child: Column(
        ...
        children: [
          ...
          PTTextField(
            ...
            label: 'Your nickname',
            errorText: _showErrors && !_nickValid ? 'Nick must not be empty' : null,
          ),
          const SizedBox(height: 32),
          _ColorPicker(
            ...
            errorText: _showErrors && !_colorsValid ? 'Colors must be picked' : null,
          ),
          const SizedBox(height: 24),
          PTElevatedButton(
            caption: 'Ready!',
  ...
  bool get _colorsValid => setEquals(_FormPage._leanCodeColors, _selectedColors);
...
class _AnimatedTimerState extends State<_AnimatedTimer> with SingleTickerProviderStateMixin {
        ...
        if (selected)
          Positioned(
            ...
            child: Icon(
              Icons.check_circle,
              size: 22,
              color: color == PTColors.lcBlack ? PTColors.lcWhite : PTColors.lcBlack,
            ),
          ),
      ],
    );
